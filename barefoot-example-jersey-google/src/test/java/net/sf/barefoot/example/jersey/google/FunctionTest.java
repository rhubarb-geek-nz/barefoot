...
package net.sf.barefoot.example.jersey.google;
...
public class FunctionTest {
  ...
  public void testSimpleGetExample() throws Exception {
    ...
    addHeader(headers, "Host", "localhost:8181");
    addHeader(headers, "Accept", "*/*");
    addHeader(headers, "User-Agent", "curl/7.55.1");

    HttpRequest req =
        ConcreteHttpRequest.builder()
            .path("/api/HttpExample")
            .uri("http://localhost:8181/api/HttpExample")
            .headers(headers)
            .method("GET")
    ...
    Assert.assertEquals(map.get("method"), "GET");
  }

  @Test
  public void testSimpleGetNotFound() throws Exception {
    Function instance = new Function();
    Assert.assertNotNull(instance);
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
